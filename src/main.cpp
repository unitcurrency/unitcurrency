...
unsigned int nTargetSpacing = 1 * 60; // UniversalCurrency - 1 minute blocks
static const int64_t nInterval = nTargetTimespan / nTargetSpacing;
static const int64_t nDiffChangeTarget = 1;

unsigned int nStakeMinAge = 1 * 60 * 60; // 1 hours
unsigned int nStakeMaxAge = -1;  // unlimited
unsigned int nModifierInterval = 10 * 60; // UniversalCurrency - time to elapse before new modifier is computed
...
const string strMessageMagic = "UniversalCurrency Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
    ...
    if(pindexBest->nHeight+1 <= 100)
    {
        ...
        else if(pindexBest->nHeight+1 >= 101  && pindexBest->nHeight+1 <= 210101)
    {
        nSubsidy = 100 * COIN;
    }
        else if(pindexBest->nHeight+1 >= 210102)
    {
        nSubsidy = 5 * COIN;
    }
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig) const
{
    ...
    if (IsProofOfStake())
    {
        ...
        // UniversalCurrency: check proof-of-stake block signature
...
// UniversalCurrency: attempt to generate suitable proof-of-stake
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "UniversalCurrency", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "UniversalCurrency 2015";
        CTransaction txNew;
        txNew.nTime = 1430067927;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1430067927;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 1709179;
       ...
       if (false && (block.GetHash() != hashGenesisBlock)) {
        ...
        assert(block.hashMerkleRoot == uint256("0x365b59809dee2d3884f516ecbbe8f8788a498c4378ed26645ba856fceae6bfd2"));
